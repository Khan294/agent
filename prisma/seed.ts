...
const agents: SeedAgent[] = [
  {
    ...
    toolsConfig: {
      allowedTools: ["check_availability", "reserve_appointment", "cancel_appointment"]
    },
    frontendConfig: {
  ...
  },
  {
    ...
    modelName: "gemma4:e2b-it-qat",
    toolsConfig: {
      allowedTools: ["list_menu", "build_cart", "place_order", "check_order_status"]
    },
    frontendConfig: {
...
async function main() {
  for (const agent of agents) {
    ...
    await prisma.ragDocument.deleteMany({ where: { agentKey: agent.agentKey } });
    ...
    for (const document of agent.documents) {
      ...
      const embedding = await embedText(agent.modelProvider, `${document.title}\n${document.content}`).catch(() => null);
      if (embedding?.length) {
        await prisma.$executeRawUnsafe(
          `UPDATE rag_documents SET embedding = $1::vector WHERE id = $2::uuid`,
